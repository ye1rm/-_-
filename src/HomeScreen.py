...
def render_home_screen(screen, font, game_font):
    ...
    # 게임 제목 텍스트 렌더링
    total_text_width = sum(font.render(char, True, YELLOW if char.isupper() else TEXT_COLOR).get_width() for char in tittle_text)
    x_pos = (WIDTH - total_text_width) // 2  # 전체 텍스트의 너비를 기준으로 중앙에 배치

    for char in tittle_text:
        char_surface = font.render(char, True, YELLOW if char.isupper() else TEXT_COLOR)
        screen.blit(char_surface, (x_pos, HEIGHT // 4))  # 텍스트 위치 지정
        x_pos += char_surface.get_width()  # 다음 문자가 이어지도록 x_pos 업데이트
